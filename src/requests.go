...
func sendWebhookMessage(user User, stream Stream) (message_id *string) {
	...
	embed.Image.URL = strings.ReplaceAll(strings.ReplaceAll(stream.ThumbnailUrl, "{width}", "1920"), "{height}", "1080") + fmt.Sprintf("?t=%d", int(math.Floor(float64(time.Now().UnixMilli()) / 60000)))
